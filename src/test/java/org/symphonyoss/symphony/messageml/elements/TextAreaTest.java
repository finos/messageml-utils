...
public class TextAreaTest extends ElementTest {
  ...
  public void testTextAreaWithRequiredAttributesOnly() throws Exception {
    ...
    verifyTextAreaPresentation((TextArea) textArea, NAME_VALUE, false, false, false, false, null);
  ...
  public void testTextAreaWithInitialValue() throws Exception {
    ...
    verifyTextAreaPresentation((TextArea) textArea, NAME_VALUE, true, false, false, false, null);
    verifyTextAreaMarkdown(null);
  }
  ...
  private void verifyTextAreaPresentation(TextArea textArea, String name,
    ...
    if (shouldRenderRequired) {
      ...
      assertNull("Required attribute should be null", textArea.getAttribute(REQUIRED_ATTR));
    }

    if (shouldRenderPlaceholder) {
      assertEquals("Placeholder attribute", String.valueOf(expectedPlaceholder), textArea.getAttribute(PLACEHOLDER_ATTR));
    } else {
      assertNull("Placeholder attribute should be null", textArea.getAttribute(PLACEHOLDER_ATTR));
    }
  }

  private void verifyTextAreaMarkdown(String placeholder) {
    String markdown = context.getMarkdown();
    String expectedMarkdown = getExpectedTextAreaMarkdown(placeholder);
  ...
  private String getRequiredTextAreaPresentationML(String requiredValue) {
    if (Arrays.asList("true", "false").contains(requiredValue)) {
